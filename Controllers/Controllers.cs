using System;

namespace PisFirst.Controllers
{
    internal class Controllers
    {
        public void Authorization(string login, string password)
        {
            throw new NotImplementedException();
        }

        public void AddAccountCard(string recordId, params string[] p)
        {
            throw new NotImplementedException();
        }

        public void EditRecordCard(string recordId, params string[] p)
        {
            throw new NotImplementedException();
        }

        public void GetHistoryOfChangesAccountCard(string recordId)
        {
            throw new NotImplementedException();
        }

        public void ShowTrappingApplications()
        {
            throw new NotImplementedException();
        }

        public void ShowRecordCard(string recordId)
        {
            throw new NotImplementedException();
        }

        public void ExportExcelRegistryRecords(string[] filters, string[] sorts)
        {
            throw new NotImplementedException();
        }

        public void ExportExcelTrappingApplications(string[] filters, string[] sorts)
        {
            throw new NotImplementedException();
        }

        public void ExportWordTrappingApplications(string[] filters, string[] sorts)
        {
            throw new NotImplementedException();
        }

        // public void FilterRecords(string[] filters, string[] sorts)
        // {
        //     throw new NotImplementedException();
        // }
    }
}
